...
function filterSuggestions (query, suggestions, length, labelKey) {
  const regex = new RegExp(`(?:^|\\s)${escapeForRegExp(query)}`, 'i')
  return suggestions.filter((item) => regex.test(item[labelKey])).slice(0, length)
}
...
class Suggestions extends React.Component {
  constructor (props) {
    ...
    this.state = {
      options: filterSuggestions(this.props.query, this.props.suggestions, this.props.maxSuggestionsLength, this.props.labelKey)
    }
  }

  componentWillReceiveProps (newProps) {
    this.setState({
      options: filterSuggestions(newProps.query, newProps.suggestions, newProps.maxSuggestionsLength, newProps.labelKey)
    })
  }
    ...
    const options = this.state.options.map((item, i) => {
      ...
      return (
        <li
          ...
          <span dangerouslySetInnerHTML={markIt(item[this.props.labelKey], this.props.query)} />
        </li>
      )
    })
