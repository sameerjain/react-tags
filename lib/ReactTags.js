...
class ReactTags extends React.Component {
  ...
  handleKeyDown (e) {
    ...
    const { delimiters, delimiterChars, labelKey } = this.props
    ...
    if (delimiters.indexOf(e.keyCode) > -1 || delimiterChars.indexOf(e.key) > -1) {
      ...
      if (query.length >= this.props.minQueryLength) {
        ...
        const match = this.suggestions.state.options.findIndex((suggestion) => {
          return suggestion[labelKey].search(new RegExp(`^${query}$`, 'i')) === 0
        })
  ...
  render () {
    ...
    const tags = this.props.tags.map((tag, i) => (
      <TagComponent
        key={i}
        tag={tag}
        classNames={this.state.classNames} labelKey={this.props.labelKey}
    ...
    var suggestions = this.props.suggestions.filter(function (suggestion) { 
      var foundArray = this.props.tags.filter(function (tag) {
    ...
    return (
      <div className={classNames.join(' ')} onClick={this.handleClick.bind(this)}>
        ...
        <div
          ...
          <Suggestions {...this.state} labelKey={this.props.labelKey}
